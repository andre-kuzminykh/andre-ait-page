...
<style>
  ...
  *, *::before, *::after{ box-shadow:none !important; text-shadow:none !important; }
  ...
  /* ===== Блок 1: о курсе ===== */
  .float-row{display:flex;justify-content:center;align-items:flex-end;gap:.85rem;transform:scale(.82);margin:0 0 .25rem}
  @media(min-width:640px){.float-row{transform:none;margin-bottom:.85rem}}
  ...
  @media (max-height:700px){
    .float-row{transform:scale(.7);margin-bottom:0}
    ...
    .cta-photo{width:min(11rem,26vh)}
  ...
  <script>
    ...
    (function(){
      ...
      function fit(p){
        ...
        var k = avail / need;
        if (k < 0.5){ k = 0.5; p.style.overflowY = 'auto'; }   // страховка: не режем
        w.style.transformOrigin = 'top center';
        w.style.transform = 'scale(' + k + ')';
        w.style.marginTop = Math.max(0, (avail - need * k) / 2) + 'px';
      ...
      function show(n){
        ...
        if (history.replaceState) history.replaceState(null, '', '#' + panels[idx].id);
      }
      ...
      function jump(n){
        var t = now();
        if (t < busyUntil){ pending = n; return; }
        if (n < 0 || n > panels.length - 1 || n === idx) return;
        show(n);
        busyUntil = t + SWITCH;
        firedAt = t;
        setTimeout(function(){
      ...
      window.addEventListener('wheel', function(e){
        ...
        else if (!armed && a >= prevAbs && a >= 12 && (t - firedAt) > REARM){ armed = true; accum = 0; }
        prevAbs = a;
        if (!armed) return;
        accum += d;
