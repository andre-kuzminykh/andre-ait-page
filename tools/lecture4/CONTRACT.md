# Контракт вёрстки слайда лекции 4

Ты верстаешь ОДИН слайд презентации. Результат — кусок HTML, который вставят
в `automation/4/index.html` между соседними слайдами. Никакого другого вывода.

Канон целиком: `/home/user/andre-ait-page/LECTURE-GUIDE.md` (§2, §4 — обязательно).
Образцы, на которые надо смотреть глазами: `/home/user/andre-ait-page/automation/3/index.html`,
слайды лежат подряд начиная со строки 2054 (`id="slide-0"`).
...
* Файл записать в `/home/user/andre-ait-page/build/l4/out/slide-NN.html`
...
9. Имена иконок — только существующие Phosphor (список ниже). Выдуманное имя
   рисуется пустотой.
10. Латиница только в именах методов и протоколов (Self-Ask, ReAct, Reflexion,
    Plan-and-Solve, Tree-of-Thought, Graph-of-Thought, Skeleton-of-Thought,
    Program-of-Thought, RAG, GraphRAG, BM25, MCP, A2A, Model Context Protocol).
    Всё остальное — по-русски. Запрещены слова: workflow, reasoning, retrieval,
    fallback, polling, guardrail, governance, throughput, human review,
    backlog, adoption, baseline, продакшен, чекпоинт, снапшот. Отдельно:
    сочетание букв «AI» запрещено, пишем «ИИ».
...
`a-step` + `a-step-2`/`a-step-3` (подсветка узлов цикла по очереди).
...
Разрешены любые существующие имена Phosphor. Проверенный набор под эту лекцию:
