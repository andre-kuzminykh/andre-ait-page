...
def build(lecture, out_path):
    ...
    fit = re.search(r"window\.__FIT\s*=\s*\{.*?\};", html, re.S)
    if not fit:
        sys.exit("не найден window.__FIT — нечего центровать")
    html = html.replace(fit.group(0),
                        "window.__FIT = { pc: { top: 28, bottom: 28 },\n"
                        "                 mob: { top: 16, bottom: 16 }, tightW: true };")
    steps.append("полосы холста заменены симметричным полем (центровка)")
