...
def сияние(подпись):
    """Ширина облака за заголовком: по слову, а не одна на всех.

    Владелец про «Данные» и «Модели»: «слишком большое свечение
    относительно текста». Короткое слово получает короткое облако."""
    голая = подпись.replace("&amp;", "&")
    return int(max(520, min(900, ШРИФТ.ширина(голая, КЕГЛЬ_ЗАГОЛОВКА) + 280)))
...
/* Фиолетовое сияние на фиолетовой стене почти не читается, поэтому у
   ...
   Размер облака НЕ фиксированный: у короткого слова («Данные») широкое
   сияние выглядело пятном втрое больше текста — владелец это и заметил.
   Ширину считает сборщик по ширине слова, она приезжает в style. */
.block .head .glow{
    height:250px;
...
/* Чипы — просто слова: круги под ними и были теми «иконками», от
   ...
   Больше четырёх слов в строку не помещается, поэтому длинные наборы
   идут в ДВА РЯДА (владелец: «в два ряда можно»). */
.block .chips{display:flex;flex-direction:column;gap:14px;align-items:center;margin-top:30px}
.block .line{display:flex;gap:34px;justify-content:center}
.block .chip{
...
/* Владелец: «все эмодзи вылетающие сделай 3-4 секунды». Один проход,
   а не бесконечный цикл: с задержками 0 / 0.55 / 1.1 последний значок
   гаснет на 3.7-й секунде после появления заголовка. */
.scene.on .fly.on i{animation:flyOut 2.6s ease-out 1 both}
@keyframes flyOut{
...
def блок(sid, in_, out_, icon, colour, подпись, шапка, шапка_cur, чипы, фразы=""):
    ...
    т = ['        <!-- ══ %s ══ -->\n' % подпись,
         ...
         '                <div class="chips">\n']
    # больше четырёх слов в строку не влезает — режем на два ряда поровну
    ряды = [чипы] if len(чипы) <= 4 else [чипы[:(len(чипы) + 1) // 2], чипы[(len(чипы) + 1) // 2:]]
    н = 0
    for ряд in ряды:
        т.append('                    <div class="line">\n')
        for t, _, текст, окно in ряд:
...
A('            </div>\n')
A(cores(core("4.25", "Без измерения невозможно управлять", "ember")))
A('        </section>\n\n')
...
A(блок("s5", "25.6", "47.2", "compass-tool", "solar", "Стратегия и управление",
       ...
       [("32.55", "target", "Цели", "32.55 34.05"),
        ...
        ("40.59", "compass-tool", "ИИ-стратегия", "40.59 41.49"),
        ("41.49", "buildings", "Структура управления", "41.49 42.87")],
...
A(блок("s7", "63.5", "83.9", "cpu", "solar", "Инфраструктура",
       ...
       [("67.71", "buildings", "Фундамент", "67.71 70.19"),
        ("71.61", "cpu", "Железо", "71.61 74.31"),
        ("74.31", "database", "Доступ к данным", "74.31 75.65"),
        ("75.65", "wrench", "Инструменты", "75.65 76.60"),
...
A(блок("s8", "84.0", "104.4", "database", "ember", "Данные",
       ...
       [("89.43", "star", "Актив", "89.43 90.45"),
        ...
        ("96.17", "database", "Хранение", "96.17 97.57"),
        ("100.41", "scales", "Принятие решений", "100.41 101.33")],
...
A(блок("s9", "104.6", "123.5", "brain", "solar", "Модели",
       ...
       [("109.01", "sparkle", "Интеллект", "109.01 110.25"),
        ...
        ("118.63", "lightning", "Скорость решений", "118.63 120.77"),
        ("121.15", "check-circle", "Качество", "121.15 121.83"),
...
A(блок("s10", "123.7", "141.1", "gear-six", "ember", "Внедрение ИИ",
       ...
       [("126.53", "arrows-merge", "В процессах", "126.53 128.83"),
        ...
        ("134.37", "target", "Результат", "134.37 135.67"),
        ("136.35", "chart-pie-slice", "Доля процессов", "136.35 137.89"),
        ("138.61", "users-three", "Сотрудники", "138.61 139.95"),
        ("139.95", "users", "Опыт клиентов", "139.95 140.85")]))
