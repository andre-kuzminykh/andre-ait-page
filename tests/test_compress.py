...
def test_archive_mode_keeps_the_picture_as_it_was():
    ...
    кадр не уменьшается, частота кадров своя, качество держит только CRF."""
    b = _body()
    ...
    assert "CRF=18" in b, "в архиве нужен CRF порога незаметности, а не веб-качество"


def test_archive_mode_has_no_bitrate_ceiling():
    """Потолок битрейта — инструмент кратности. В архиве он и есть «заметная
    потеря»: на движении и зерне картинка посыпалась бы ровно в потолок."""
    b = _body()
    assert '[ "$ARCHIVE" = 1 ] && CAP=""' in b, "в архиве потолок битрейта не снимается"
    assert "$CAP" in b, "потолок не вынесен в переменную — снять его нечем"
