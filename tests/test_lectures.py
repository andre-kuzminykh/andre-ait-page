...
def test_process_practice_viewer():
    ...
    # у кнопки «со схемой» два класса, и общий обработчик копирования не должен
    # цеплять её вторым: он перезаписывал подпись, и она навсегда оставалась
    # «скопировано»
    assert "querySelectorAll('.copy[data-copy]:not(.copy-code)')" in html, \
        rel + ": обработчики копирования не должны срабатывать дважды на одной кнопке"
