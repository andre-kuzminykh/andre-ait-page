...
FR-SITE1 — «Free AI Diagnostic» ведёт на maturity.andre.technology (бесплатная
           диагностика AI-зрелости — первый шаг воронки).
...
def test_free_diagnostic_links_to_maturity():
    html = _html()
    # найдём каждый элемент, содержащий текст «Free AI Diagnostic», и проверим,
    # что это ссылка <a> на maturity.andre.technology (бесплатная диагностика)
    ...
    for pos in hits:
        ...
        assert "https://maturity.andre.technology/" in el, \
            "«Free AI Diagnostic» должна вести на maturity.andre.technology: " + el[:80]


def test_free_diagnostic_never_links_to_strategy():
    html = _html()
    # диагностика — это maturity.; на strategy. ведут только стратегические CTA
    hits = [m.start() for m in re.finditer(r"Free AI Diagnostic", html)]
    for pos in hits:
        tag_start = html.rfind("<", 0, pos)
        el = html[tag_start:pos]
        if el.strip().startswith("<!--"):
            continue
        assert "strategy.andre.technology" not in el, \
            "«Free AI Diagnostic» не должна вести на strategy.andre.technology: " + el[:80]
