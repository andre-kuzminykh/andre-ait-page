...
def test_header_full_width_and_cta():
    ...
    for rel, html in _pages():
        ...
        assert re.search(r"height:3\.1rem;(?:box-sizing:border-box;)?padding:0 1\.8rem;"
                         r"font-size:16px;letter-spacing:\.13em", html), \
            rel + ": десктопная кнопка «Буткемп» — крупный кегль (заказчик дважды просил больше)"
