...
def test_script_pins_clip_tracks_to_zero():
    """Видеодорожка ролика прижимается к нулю (setpts): в MOV с телефона она
    бывает начинается позже звука, и без этого после обложки шёл чёрный кадр.
    Звук больше НЕ растягивается и не прижимается — он идёт копией со сдвигом
    на заставку (-itsoffset), исходное соотношение дорожек сохраняется само."""
    body = _html()
    body = body[body.index("function joinScript()"):body.index("var joinBtn")]
    assert "[1:v]setpts=PTS-STARTPTS" in body, "видеодорожка ролика не прижата к нулю"
    assert "-itsoffset" in body, "звук не сдвинут на длину заставки"
    assert "aresample=async" not in body, \
...
def test_audio_is_copied_when_it_can_be():
    """Владелец: «звук искажается всё равно» — после этого пережим звука в
    лекции убран совсем. Обложка нужна только картинке: пересобирается одна
    видеодорожка, а звук идёт копией из исходника (проверено md5 — бит-в-бит),
    просто начинается на длину заставки позже. Пережим остаётся только для
    не-AAC дорожек: их в mp4 копией не положить."""
    html = _html()
    script = html[html.index("function joinScript()"):html.index("var joinBtn")]
    ...
    assert "-b:a 256k" in script and "-b:a 192k" not in script, "остался старый пережим"
