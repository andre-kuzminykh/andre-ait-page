...
def test_no_cue_wraps_to_a_second_line():
    """Владелец: «субтитры наслаивались друг на друга».

    Наслаивались не соседние реплики — на две строки уходила ОДНА:
    «способны AI-агенты, важно посмотреть» это четыре слова и 1120px при
    полосе 940. Меряем по метрикам шрифта (оценка сверху: кернинг и
    letter-spacing делают строку чуть уже) и требуем, чтобы влезало и в
    полосу страницы (940), и в полосу libass при вжигании (960)."""
    ш = _шрифт()
...
def test_cut_checks_the_label_on_the_finished_file():
    """Владелец: «мне надо, чтобы текст наклеивался в процессе сборки».
...
def test_cut_prints_its_version():
    """Старая копия скрипта в папке уже стоила часа разбирательств: лог
    выглядел «нормально», просто это был не тот файл."""
    body = _cut()
    assert body.index("echo \"нарезать.sh v") < body.index("FF=\"\""), \
...
def test_cut_trusts_ready_made_numbered_covers():
    """cover1, cover2… — это ГОТОВЫЕ обложки частей: подпись на них уже есть,
    вторую поверх рисовать нечего. Это же спасает, когда ffmpeg рисовать
    текст не умеет вовсе."""
    body = _cut()
    assert "NUMCOVERS=0" in body, "готовые обложки частей не распознаются"
    assert '[ "$CAPSET" = 0 ]' in body, "явное подпись=… не перебивает это правило"
    assert "на них она уже есть" in body, "молча меняет поведение, не сказав в логе"
