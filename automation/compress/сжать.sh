...
#   sh сжать.sh x10             — ровно во столько раз, чего бы это ни стоило
#                                 (x6, x20 — любое число от двух). Обычные
#                                 режимы держат нижний порог качества, этот —
#                                 кратность, и честно предупреждает про мыло
set -e
...
case "${1:-}" in
    ...
    # «Сожми ровно в N раз»: sh сжать.sh x10 (или х10 кириллицей).
    # У режимов выше кратность — цель, а нижний порог битрейта (MINK) её
    # придерживает: на исходнике 5 Мбит/с обычный режим даёт 6 раз, а не
    # 12, потому что ниже 700 кбит/с он не опускается. Здесь кратность
    # ГЛАВНЕЕ: порог снят почти до нуля, и если цена этому — мыло, скрипт
    # говорит об этом прямо, а не молчит.
    [xх][0-9]*)
        RATIO=$(printf '%s' "${1#?}" | tr -cd '0-9')
        [ -n "$RATIO" ] && [ "$RATIO" -ge 2 ] 2>/dev/null || {
            echo "Кратность пишется как x10 — во сколько раз сжимать (от 2)."; exit 1; }
        MINK=120; MAXCAP=8000; SKIPK=0; shift ;;
esac
...
for f in "$@"; do
    ...
    # Когда кратность заказана явно (режим x10), звук становится полом:
    # на лёгком исходнике видео режется до 300 кбит/с, а 96k звука — это
    # уже треть файла, и десятикратность недостижима. Ужимаем и его —
    # соразмерно картинке, но не ниже разборчивости речи.
    if [ -n "$ACODEC" ] && [ "$SKIPK" = 0 ] && [ "$ARCHIVE" = 0 ]; then
        if [ "$MAXK" -lt 300 ]; then AOPT="-c:a aac -b:a 48k -ac 1"
        elif [ "$MAXK" -lt 600 ]; then AOPT="-c:a aac -b:a 64k -ac 1"
        fi
    fi
    ...
    # Честное предупреждение о мыле. Имена внутри awk — латиницей: он,
    # как и dash, кириллические не принимает («unexpected character 0xd0»).
    # Считаем биты на пиксель ВЫХОДНОГО
    # кадра: ниже 0.04 картинка сыпется на любом движении, и молчать об
    # этом нельзя — заказанная кратность важнее, но знать цену надо.
    # (Кадр выхода: короткая сторона SHORT, если исходник крупнее.)
    if [ "$REMUX" = 0 ]; then
        BLURRY=$(awk -v w="$DW" -v h="$DH" -v f="$FPS" -v k="$MAXK" -v s="$SHORT" \
            'BEGIN{ sm=(w<h?w:h); if (sm>s) { w=w*s/sm; h=h*s/sm }
                    bpp=(w*h*f>0) ? k*1000/(w*h*f) : 1;
                    printf "%d", (bpp<0.04) ? 1 : 0 }')
        [ "$BLURRY" = 1 ] && NOTE="$NOTE [${MAXK} кбит/с — картинка станет мыльной]"
    fi
