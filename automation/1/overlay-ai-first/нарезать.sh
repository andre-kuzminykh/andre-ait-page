...
# Средняя яркость полоски, где стоит подпись. Полоска усредняется в ОДИН
# пиксель (scale=1:1) — буквы белые, поэтому с подписью она заметно светлее.
# Так проверяется готовый файл: пробный кадр показывает, что фильтр умеет
# рисовать, а это — что подпись реально доехала до ролика.
STRIPH=$(awk -v s="$CAPSIZE" 'BEGIN{ printf "%d", s*1.6 }')
stripmean() {   # <файл> [ключи перед -i, например «-ss 0.02»]
  "$FF" -v error $2 -i "$1" -frames:v 1 \
    -vf "scale=$DW:$DH,crop=$DW:$STRIPH:0:$CAPY,scale=1:1,format=gray" \
...
for A in $STARTS; do
  ...
  # Подпись — на ГОТОВОМ файле, а не только на пробном кадре. Полоску,
  # где она стоит, усредняем в один пиксель (scale=1:1) в самом ролике и в
  # исходной обложке: буквы белые, поэтому в ролике полоска светлее. Если
  # разницы нет — подпись не доехала, и надо об этом сказать, а не отдавать
  # молча куски без «Часть N».
  if [ "$CAP" = 1 ]; then
    before=$(stripmean "$PIC")
    after=$(stripmean "$OUT" "-ss 0.02")
    if [ -n "$before" ] && [ -n "$after" ]; then
      awk -v a="$before" -v b="$after" -v n="$i" -v w="$CAPWORD" 'BEGIN{
        d = b - a; if (d < 0) d = -d;
        if (d < 2) printf "  ⚠︎ часть %s: подписи «%s %s» в кадре НЕТ — полоска не посветлела (%d→%d)\n", n, w, n, a, b;
        else       printf "  подпись «%s %s» в кадре (полоска %d→%d)\n", w, n, a, b }'
    fi
  fi
