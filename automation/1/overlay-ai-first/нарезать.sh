...
# Сколько ЯРКИХ пикселей в полоске, где стоит подпись. Буквы белые, так
# что с подписью их заметно больше. Считаем именно пиксели: усреднить
# полоску в один пиксель (scale=1:1) нельзя — swscale при таком сжатии не
# усредняет, а выбирает, и белые буквы в среднее не попадали вовсе.
# od -v обязателен: без него повторяющиеся строки схлопываются в «*», и
# счёт выходит меньше настоящего.
STRIPH=$(awk -v s="$CAPSIZE" 'BEGIN{ printf "%d", s*1.6 }')
stripink() {   # <файл> [ключи перед -i, например «-ss 0.02»]
  "$FF" -v error $2 -i "$1" -frames:v 1 \
    -vf "scale=$DW:$DH,crop=$DW:$STRIPH:0:$CAPY,format=gray" -f rawvideo - 2>/dev/null \
    | od -An -v -tu1 | awk '{ for(i=1;i<=NF;i++) if ($i>200) n++ } END{ print n+0 }'
}

captest() {   # <режим> → 0, если на чёрном кадре появились светлые пиксели
  # Имена переменных латиницей: dash кириллические не принимает.
  rm -f "$TMP/probe.png"
  "$FF" -y -v error -f lavfi -i "color=c=black:s=${DW}x${DH}" -frames:v 1 \
      -vf "null$(capfilter 1 "$1")" "$TMP/probe.png" 2>/dev/null || return 1
  [ -f "$TMP/probe.png" ] || return 1
  ink=$(stripink "$TMP/probe.png")
  [ -n "$ink" ] || return 1
  [ "$ink" -gt 50 ] 2>/dev/null
}
CAPMODE=none
if [ "$CAP" = 1 ]; then
...
if [ "$CAP" = 1 ]; then
  echo "Подпись: «$CAPWORD 1»…«$CAPWORD $N», кегль $CAPSIZE, от верха кадра $CAPY"
  echo "  способ: $CAPMODE, шрифт: $(basename "$FONTNAME"), проверено на пробном кадре"
fi
...
for A in $STARTS; do
  ...
  # Подпись — на ГОТОВОМ файле, а не только на пробном кадре. Считаем
  # светлые пиксели полоски в самом ролике и в исходной обложке: буквы
  # белые, значит в ролике их заметно больше. Разницы нет — подпись не
  # доехала, и надо сказать об этом, а не отдавать молча куски без
  # «Часть N».
  if [ "$CAP" = 1 ]; then
    before=$(stripink "$PIC")
    after=$(stripink "$OUT" "-ss 0.02")
    if [ -n "$before" ] && [ -n "$after" ]; then
      awk -v a="$before" -v b="$after" -v n="$i" -v w="$CAPWORD" 'BEGIN{
        if (b - a < 100) printf "  ⚠︎ часть %s: подписи «%s %s» в кадре НЕТ — светлых пикселей столько же (%d→%d)\n", n, w, n, a, b;
        else             printf "  подпись «%s %s» в кадре (+%d светлых пикселей)\n", w, n, b-a }'
    fi
  fi
