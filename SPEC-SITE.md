...
## FR-SITE11 — логотип из /assets, не с внешнего хостинга

**Story.** Как владелец, я хочу, чтобы логотип грузился из репозитория
(`/assets/_ait_logo.png`), а не с i.ibb.co.

**Требование.** Во всех страницах (index.html и automation/*, включая лекции
1 и 2) `<img>` логотипа указывает на `/assets/_ait_logo.png`; строки `i.ibb.co`
нет нигде. Ссылка вида `github.com/.../blob/...` НЕ годится — это HTML-страница,
а не картинка; сайт обслуживается GitHub Pages из этого же репозитория, поэтому
корневой путь `/assets/...` работает на всех страницах.

**Тесты:** test_logo_self_hosted (в test_site.py и test_automation.py).
