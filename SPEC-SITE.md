...
**Известный пробел (НЕ баг).** Для лекции 6 («AI-агент в продакшене») главы в
книге нет: её темы (доверие и UX агента, промпт-инъекции, канареечные релизы,
SLA агента) в тексте не встречаются ни разу. Панель в лекцию 6 вставлена, но
данные пустые — скрипт панели в этом случае сам скрывает кнопку. Когда текст
появится, достаточно подложить данные в `<script id="slide-notes">`.
...
**Сверка текста с книгой.** Скрипт сверяет каждый фрагмент панели с текстом
книги и каждый абзац книги с панелями (нормализация без регистра, пунктуации и
пробелов; допускаются снятие порядкового префикса при свёртке в список и склейка
соседних предложений во врезку). Результат: 1718 фрагментов панелей и 1681
абзац книги — выдуманного 0, потерянного 0. Расхождения на швах перечислений
(«Первая — постановка целей» → пункт 1 «Постановка целей») смыслом не являются:
порядковый связки заменён номером карточки.
