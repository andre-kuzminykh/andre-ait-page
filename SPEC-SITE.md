...
- Списки оформляются точками/цифрами в фирменных цветах, маркеры соседних
  списков чередуются фиолетовый ↔ оранжевый.
- Десктоп (≥1024px): панель шириной `clamp(340px,33vw,560px)` справа; слайд,
  шапка и прогресс ужимаются на её ширину (инфографика уезжает влево), стрелки
  листания сдвигаются левее панели. Слова в узком слайде НЕ рвутся посреди —
  вписывание берёт на себя `portal-fit` (zoom).
- Мобилка (<1024px): панель на весь экран поверх всего; кружок с говорящей
  головой прячется (звук идёт), у текста запас снизу 96px под стрелки.
- Стрелки листания — поверх панели (`z-index:9600`); смена слайда обновляет
  текст и прокручивает его наверх, но НЕ закрывает панель.
- Закрытие — крестиком слева сверху панели или клавишей Esc.
