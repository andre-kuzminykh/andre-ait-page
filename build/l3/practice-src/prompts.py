...
У ИИ-операции и у агента одинаково есть контекст — входные данные для модели. Агент отличается тем, что у него есть ещё набор инструментов, которые он вызывает сам, и лимит итераций.
...
3) Узлы — таблица: | ID | Навык | Чем выполняется | Контекст: данные и источник | Инструменты агента |. Контекст — у ИИ-операций и агентов: минимально достаточный набор данных, а не всё подряд. Инструменты — только у агентов: узкие функции вроде «написать клиенту» или «найти в регламенте», а не «управлять CRM».
...
- Агент — узел в форме [[...]]: "<b>Уточнить у клиента</b><br/>агент: почта, CRM<br/>до 2 кругов". Внутрь не раскладывай и петлю на себя не рисуй — порядок шагов агент выбирает сам. Выходы агента подпиши по полю его вывода, например -->|ответы получены| и -->|2 круга без ответа|; один из них всегда ведёт в «Передано человеку».
...
- инструмент: система;
- человек;
- агент: его инструменты и лимит итераций.
...
\"\"\"
СЮДА ВСТАВЬТЕ РЕЗУЛЬТАТ ПРОМТА 1
\"\"\"
