# -*- coding: utf-8 -*-
"""Замер ОДНОГО или нескольких слайдов лекции 3 в обеих формах + кадры.

    python3 build/l3/probe.py 7            # слайд 7
    python3 build/l3/probe.py 7 8 9        # несколько
    python3 build/l3/probe.py 7 --json     # машинный вывод

Собирает пробную колоду из build/l3/out (меряемые слайды целиком, остальные —
заглушками), со СВОИМ собранным CSS (параллельные прогоны не мешают друг другу),
открывает её в форме компьютера 1380×864 и телефона 376×844 и печатает:

* масштаб — во сколько раз подгонщик растянул/сжал холст слайда (норма ПК ≥ 0.95:
  меньше — слайд переполнен и выглядит мелко; кап подгонщика 1.3);
* заполнение — какую долю полезной площади занимает содержимое;
* минимальный эффективный кегль на телефоне (норма ≥ 10px);
* заступы за края экрана и за рамку карточки (норма 0);
* аварийные разрывы слов посреди (норма 0);
* наложения текста на текст (норма 0);
* многострочные тексты: число строк и ВИСЯЧЕЕ одно слово на последней строке
  (владелец «очень трепетно относится к переносам»); подзаголовок обязан быть
  одной строкой на ПК.

Кадры: build/l3/frames/slide-NN-pc.png и slide-NN-mob.png — их надо СМОТРЕТЬ.
Запускать из корня репозитория.
"""
import io, json, os, re, shutil, socket, subprocess, sys, tempfile

ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
L6 = os.path.join(ROOT, "build", "l3")
...
def build_probe(ids):
    ...
    src = os.path.join(L6, "out")
    tmp = tempfile.mkdtemp(prefix="l3probe-")
    have = set()
    ...
    for i in range(max(have | {41}) + 1):
        if i not in have:
            io.open(os.path.join(tmp, "slide-%02d.html" % i), "w", encoding="utf-8").write(STUB % (i, i))
    tag = "_probe_%d" % os.getpid()
    page = os.path.join(ROOT, "automation", "3", tag + ".html")
    css = os.path.join(ROOT, "assets", tag + ".css")
    os.environ["L3_OUT"] = tmp
    os.environ["L3_DST"] = page
    import assemble
    ...
    try:
        ...
        sys.stdout = old
    # свой CSS: содержимое — только меряемые слайды + каркас (быстро и без гонок)
    import css as cssmod
    cssmod.build(css, [os.path.join(tmp, "*.html"), os.path.join(ROOT, "automation", "4", "index.html")])
    html = io.open(page, encoding="utf-8").read()
    html = html.replace('href="/assets/lecture-3.css"', 'href="/assets/%s.css"' % tag, 1)
...
def main():
    ...
    try:
        with sync_playwright() as pw:
            ...
            for form, vp in (("pc", lc.PC), ("mob", lc.MOB)):
                ...
                page = ctx.new_page()
                page.goto("http://127.0.0.1:%d/automation/3/%s" % (rl.PORT, name),
            ...
            if light and shots:
                for form, vp in (("pc", lc.PC), ("mob", lc.MOB)):
                    ...
                    if v:
                        ctx.route("**/*", rl.vendor_route(v))
                    page = ctx.new_page()
                    page.goto("http://127.0.0.1:%d/automation/3/%s?theme=light" % (rl.PORT, name),
                              wait_until="load", timeout=120000)
                    page.wait_for_timeout(3000)
                    page.evaluate("() => document.querySelectorAll('#start-overlay,#intro-overlay').forEach(e => e.remove())")
                    for i in ids:
    ...
    for i in ids:
        ...
        if shots:
            print("   кадры: build/l3/frames/slide-%02d-pc.png, slide-%02d-mob.png%s"
                  % (i, i, " (+ -pc-light, -mob-light)" if light else ""))
